...
def input_data(path):
    ...
    data = data[['prospectid', 'Mobile Number','Phone Number', 'Email', 'Primary Source Campaign', 'eKYC Stage Code', 'Total Payment Amount', 'Created On']]
...
def creating_lead_stage(data_merged):
    ...
    #data_merged['Total Payment Amount'].fillna(0, inplace=True)  # this was as per old data
    ...
    # Extracting Dates from timestamp of 'Created On' column
    #data_merged['Created On'] = pd.to_datetime(data_merged['Created On']).dt.date  # this was as per old data
...
#     # Extracting Dates from timestamp of 'Created On' column
#     data_merged['Created On'] = pd.to_datetime(data_merged['Created On']).dt.date
